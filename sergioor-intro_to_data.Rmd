...
-   Make a scatterplot of weight versus desired weight. Describe the 
    ...
    **Seems to be a fairly strong linear relationship between th two variables**
...
-   Let's consider a new variable: the difference between desired weight 
    ...
    ```{r newv}
    wdiff=as.numeric(cdc$wtdesire-cdc$weight)
    df.cdc<-cbind(cdc, wdiff)
...
-   What type of data is `wdiff`? If an observation `wdiff` is 0, what does 
    ...
    **wdiff is integer**
    **if difference is 0 means that specific person is in their desired weight**
    **if negative is the amount away from the desired weight and positive is how far below they're from the desired weight**
...
-   Describe the distribution of `wdiff` in terms of its center, shape, and 
    ...
    hist (df.cdc$wdiff)
    abline(v=mean(df.cdc$wdiff), col='red')  
    boxplot(wdiff)
  

    ```

    **Distribution is normally distributed, central shape unimodal showing some outliers so mean and standard deviatio will be the best data points for analysis**
...
-   Using numerical summaries and a side-by-side box plot, determine if men tend
    ...
    ```{r side}
       ...
       men.wdiff <- subset(df.cdc, gender == "m", select = c(wtdesire, weight))
       summary(men.wdiff)
       women.wdiff <- subset(df.cdc, gender == "f", select = c(wtdesire, weight))
       summary(women.wdiff)
       boxplot(wdiff ~ df.cdc$gender, ylim = c(-75,75))
    
    ```
   
   
    **Females in general perceive that they should loose more weight that males and men perceive that they're closer to their desired weight**
...
-   Now it's time to get creative. Find the mean and standard deviation of 
    ...
    meanweight<-mean(df.cdc$weight)
    stdweight <- sd(df.cdc$weight)
    onesd <- subset(df.cdc, (weight < (meanweight + stdweight)) & (weight > (meanweight - stdweight)))
    percentage <- (dim(onesd)[1]/dim(cdc)[1])*100
    
    
    #standard normal distribution data
    x <- df.cdc$weight
    hx <- dnorm(x, mean=meanweight, sd=stdweight,log=FALSE)

    #plot a standard normal distribution
    plot(x, hx, type="h", lty=1, xlab="x value")
    #plot a vertical line at -2*std
    abline(v=meanweight-stdweight, col='red')
    #plot a vertical line at  2*std
    abline(v=meanweight+stdweight, col='red')
    #mean
    abline(v=meanweight,col='blue')
    #make the arrow
    arrows(x0=meanweight-stdweight, y0=0.009, x1=meanweight+stdweight, y1=0.009, code=3, col='blue')
    #plot the text
    text(x=300, y=0.004,labels=percentage, col='red')
    
    ```
    
    **70.76% Percent of the data lies between 1 Standard Deviation**
